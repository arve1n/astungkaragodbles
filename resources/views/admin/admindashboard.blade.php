...
<body>
	...
	<div id="wrapper">
			...
			<div class="container-fluid">
				...
				<div id="navbar-menu">
					<ul class="nav navbar-nav navbar-right">
						<li class="dropdown">
							@php $admin_unRead = \App\AdminNotifications::where('notifiable_id', 10)->where('read_at',
							NULL)->orderBy('created_at','desc')->count(); @endphp
							<a href="#" class="dropdown-toggle icon-menu" data-toggle="dropdown">
								<i class="lnr lnr-alarm"></i>
								<span class="badge bg-danger">@php echo $admin_unRead @endphp</span>
							</a>
							<ul class="dropdown-menu notifications">
								@php $admin_notifikasi = \App\AdminNotifications::where('notifiable_id',
								10)->where('read_at', NULL)->orderBy('created_at','desc')->get(); @endphp
								@forelse ($admin_notifikasi as $notifikasi)
								@php $notif = json_decode($notifikasi->data); @endphp
								<li>
									<a href="@if ($notif->category == 'transaction') {{ route('admin.notification', $notifikasi->id) }} @elseif ($notif->category == 'review') {{ route('admin.notification', $notifikasi->id) }} @endif"
										class="notification-item"><span
											class="dot bg-warning"></span><small>[{{$notif->nama}}]</small>
										{{$notif->message}}</a></li>
								@empty
						...
						<li class="dropdown">
							<a href="#" class="dropdown-toggle" data-toggle="dropdown"><span>pastibisa08</span> <i
									class="icon-submenu lnr lnr-chevron-down"></i></a>
							<ul class="dropdown-menu">
								<li><a href="{{ route('admin.logout') }}"><i class="lnr lnr-exit"></i>
										<span>Logout</span>
									</a></li>
							</ul>
						</li>
		...
		<div id="sidebar-nav" class="sidebar">
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						<li><a href="/admin" class="{{ Request::url() == url('/admin') ? 'active' : '' }}"><i
									class="lnr lnr-home"></i> <span>Dashboard</span></a></li>
						<li><a href="/courier" class="{{ Request::url() == url('/courier') ? 'active' : '' }}"><i
									class="lnr lnr-code"></i> <span>Courier</span></a></li>
						<li><a href="/products" class="{{ Request::url() == url('/products') ? 'active' : '' }}"><i
									class="lnr lnr-chart-bars"></i> <span>Product</span></a></li>
						<li><a href="/categories" class="{{ Request::url() == url('/categories') ? 'active' : '' }}"><i
									class="lnr lnr-cog"></i> <span>Product Categories</span></a></li>
						<li><a href="/transactions"
								class="{{ Request::url() == url('/transactions') ? 'active' : '' }}"><i
									class="fa fa-shopping-cart"></i> <span>Transactions</span></a></li>
					</ul>
				</nav>
			</div>
		</div>
					...
					<div class="panel panel-headline">
						...
						<div class="panel-body">
							...
							<div class="col-sm-12 col-md-6 text-right">
								<button class="btn btn-lg btn-block btn-primary text-uppercase"><a href="/getlaporan"
	...
	<footer>
		<div class="container-fluid">
			<p class="copyright">&copy; 2021 <a href="https://www.themeineed.com" target="_blank">Kelompok 14</a>. All
				Rights Reserved.</p>
		</div>
	</footer>
	</div>
